...
def load_custom_info():
    ...
    ignore_list = [line.split(':')[1].strip().replace('"', '') for line in lines_list if line.startswith('ignore:')]
    ...
    if len(ignore_list) != len(id_list):
        ignore_list = ['' for _ in range(len(id_list))]
    ...
    df = df[(df['remark'].notnull()) & (df['remark'] != '') | (df['ignore'].notnull()) & (df['ignore'] != '') | (
